...
class Evaluator(object):
    ...
    def eval_parser(self, files_path, measure, report=False, bcvocab=None, draw=True):
        """ Test the parsing performance"""
        # Evaluation
        met = Metrics(levels=['span', 'nuclearity', 'relation'])
        # ----------------------------------------
        # Read all files from the given path
        doclist = [os.path.join(files_path, fname) 
        ...
        for fmerge in doclist:
            ...
            fbrackets = fmerge.replace('.merge', '.brackets').replace('/merge/', '/brackets/')
            # Write brackets into file
            Evaluator.writebrackets(fbrackets, pred_brackets)
            # ----------------------------------------
            # Evaluate with gold RST tree
            if report:
                fdis = fmerge.replace('.merge', '.dis').replace('/merge/', '/discourse/')
                gold_rst = RstTree(fdis, fmerge)
                gold_rst.build()
                met.eval(gold_rst, pred_rst, measure)
